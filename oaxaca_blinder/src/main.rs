...
fn validate_output_path(path: &Path) -> Result<(), Box<dyn Error>> {
    for component in path.components() {
        if component == Component::ParentDir {
            return Err(format!(
                "Invalid output path '{}': path traversal ('..') is not permitted",
                path.display()
            )
            .into());
        }
...
mod tests {
    ...
    #[test]
    fn test_validate_output_path_safe_paths() {
        assert!(validate_output_path(Path::new("output.json")).is_ok());
        assert!(validate_output_path(Path::new("subfolder/output.json")).is_ok());
        assert!(validate_output_path(Path::new("./output.json")).is_ok());
        assert!(validate_output_path(Path::new("./subfolder/output.json")).is_ok());
    ...
    #[test]
    fn test_validate_output_path_traversal_paths() {
        assert!(validate_output_path(Path::new("../output.json")).is_err());
        assert!(validate_output_path(Path::new("subfolder/../output.json")).is_err());
        assert!(validate_output_path(Path::new("/tmp/../etc/passwd")).is_err());
        assert!(validate_output_path(Path::new("../../../etc/shadow")).is_err());
    }
}
